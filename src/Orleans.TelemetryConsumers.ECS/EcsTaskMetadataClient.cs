...
/// </summary>
internal sealed class EcsTaskMetadataClient : IEcsTaskMetadataClient
{
    /// <summary>The HTTP request timeout (in seconds).</summary>
    private const int TIMEOUT = 5;

    /// <summary>The pre-compiled error message logged when a JSON exception is caught.</summary>
    private static readonly Action<ILogger, Exception?> LogJsonException = LoggerMessage.Define(LogLevel.Error, 1, "Failed to parse ECS Task Metadata Endpoint JSON document");
    ...
    /// </exception>
    public EcsTaskMetadataClient(HttpClient? httpClient, IEcsTaskMetadataBaseAddressProvider? baseAddressProvider, IEcsTaskMetadataSerializerProvider? serializerProvider, ILogger<EcsTaskMetadataClient>? logger)
    {
        if (httpClient == null)
        {
    ...
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result returns the Docker stats or <c>null</c>
    ///     if an error occurs.
    /// </returns>
    ...
    /// <summary>
    ///     Asynchronously gets the collection of Docker stats for all containers belonging to the ECS Task running the
    ///     current process.
    /// </summary>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>
    ...
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>
    ///     A task that represents the asynchronous operation. The task result returns the deserialized JSON document or
    ...
    private async Task<T?> GetAsync<T>(string relativePath, CancellationToken cancellationToken) where T : class
    {
        ...
        try
        {
            using var jsonTextReader = new JsonTextReader(new StreamReader(await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false)))
            {
        ...
        catch (JsonException e)
        {
            if (this.logger != null)
            {
                LogJsonException(this.logger, e);
            }
